...
ui <- fluidPage(
  ...
  fluidRow(
    column(6,
           tags$h5("Genre Sales Over Time"),
           br(),
           br(),
           plotOutput("sales_time_plot")),
    
    column(6,
           tags$h5("Genre Ratings Over Time"),
           br(),
           br(),
           plotOutput("ratings_time_plot"))
  )
...
server <- function(input, output) {
  ...
  output$sales_time_plot <- renderPlot({
    game_sales %>% 
      group_by(genre, year_of_release) %>% 
      summarise(sales = round(mean(sales), 2), .groups = "keep") %>% 
      filter(genre == "Action") %>% 
  ...
  output$ratings_time_plot <- renderPlot({
    game_sales %>% 
      group_by(genre, year_of_release) %>% 
      summarise(user_score = round(mean(user_score), 2), .groups = "keep") %>% 
      filter(genre == "Action") %>% 
